...
import type { CalendarEvent, Routine, ShoppingItem, Task, TaskSubtask } from "@/lib/types";
...
export default function HouseholdReportPage(){
 ...
 const [balance,setBalance]=useState<ContributionBalanceData>({contributions:[],participants:[]}); const [shopping,setShopping]=useState<ShoppingItem[]>([]); const [events,setEvents]=useState<CalendarEvent[]>([]); const [routines,setRoutines]=useState<Pick<Routine,"id"|"household_id"|"name">[]>([]); const [tasks,setTasks]=useState<Task[]>([]); const [subtasks,setSubtasks]=useState<TaskSubtask[]>([]); const [acceptedActions,setAcceptedActions]=useState<HouseholdAcceptedAction[]>([]); const [routineAdaptations,setRoutineAdaptations]=useState<RoutineAdaptationMemorySource[]>([]); const [lifeContexts,setLifeContexts]=useState<MemberLifeContext[]>([]); const [loadPerception,setLoadPerception]=useState<LoadPerception|null>(null); const [savingPerception,setSavingPerception]=useState(false); const [ready,setReady]=useState(false); const [confirming,setConfirming]=useState(false); const [saving,setSaving]=useState(false); const [applied,setApplied]=useState(false);
 ...
 useEffect(()=>{if(!household)return;(async()=>{const [b,s,e,routineResult,taskResult,subtaskResult,actionResult,routineAdaptationResult,lifeContextResult,perceptionResult]=await Promise.all([fetchContributionBalanceData(supabase,household.id),supabase.from("shopping_items").select("*").eq("household_id",household.id).eq("status","bought"),supabase.from("calendar_events").select("*").eq("household_id",household.id).eq("visibility","household"),supabase.from("routines").select("id, household_id, name").eq("household_id",household.id),supabase.from("tasks").select("*").eq("household_id",household.id),supabase.from("task_subtasks").select("*").eq("household_id",household.id).order("position",{ascending:true}),supabase.from("household_action_suggestions").select("id, household_id, task_id, suggested_member_id, previous_assigned_to, reason, accepted_at").eq("household_id",household.id).order("accepted_at",{ascending:false}).limit(20),supabase.from("routine_adaptation_preferences").select("id, household_id, routine_id, suggested_frequency, suggested_custom_days, suggested_anchor_weekday, status, suggested_at, responded_at").eq("household_id",household.id).eq("status","accepted").order("responded_at",{ascending:false}).limit(20),supabase.from("member_life_contexts").select("*").eq("household_id",household.id),supabase.from("member_load_perceptions").select("perception, declared_at").eq("household_id",household.id).eq("member_id",me?.id||"").order("declared_at",{ascending:false}).limit(1)]);if(routineResult.error)throw routineResult.error;if(actionResult.error)throw actionResult.error;if(routineAdaptationResult.error)throw routineAdaptationResult.error;if(lifeContextResult.error)throw lifeContextResult.error;if(perceptionResult.error)throw perceptionResult.error;setBalance(b);setShopping((s.data as ShoppingItem[])||[]);setEvents((e.data as CalendarEvent[])||[]);setRoutines((routineResult.data as Pick<Routine,"id"|"household_id"|"name">[])||[]);setTasks((taskResult.data as Task[])||[]);setSubtasks((subtaskResult.data as TaskSubtask[])||[]);setAcceptedActions((actionResult.data as HouseholdAcceptedAction[])||[]);setRoutineAdaptations((routineAdaptationResult.data as RoutineAdaptationMemorySource[])||[]);setLifeContexts((lifeContextResult.data as MemberLifeContext[])||[]);setLoadPerception(((perceptionResult.data?.[0]?.perception as LoadPerception|undefined)??null));setReady(true)})().catch(err=>{console.error("DABO weekly report load failed",err);setReady(true)});},[household,me?.id,supabase]);
 ...
 const householdMemory=useMemo(()=>buildHouseholdMemory({
   householdId:household?.id||"",
   routineAdaptations,
   acceptedRebalances:acceptedActions,
  }),[household?.id,routineAdaptations,acceptedActions]);
 const householdMemoryPresentation=useMemo(()=>householdMemory.map(memory=>presentHouseholdMemory(memory,{
  routines,
  tasks,
  members,
 })),[householdMemory,routines,tasks,members]);
 ...
 return <><Header eyebrow={t("weekly_report_eyebrow")} title={t("weekly_report_title")}/><main className="mx-5 space-y-4 pb-28">
  ...
  {householdMemoryPresentation.length>0&&<section className="rounded-2xl border border-borderLight/70 bg-white2/70 p-5">
   ...
   <div className="mt-4 space-y-2">
    {householdMemoryPresentation.slice(0,3).map(memory=>{
     const subject=memory.subjectName??t(memory.type==="accepted_routine_adaptation"?"household_memory_previous_routine":"household_memory_previous_task");
     const text=memory.type==="accepted_routine_adaptation"
      ?t("household_memory_routine").replace("{subject}",subject)
      :t("household_memory_rebalance")
        .replace("{subject}",subject)
        .replace("{member}",memory.memberName??t("household_memory_unknown_member"));
     const date=new Intl.DateTimeFormat(undefined,{dateStyle:"medium"}).format(new Date(memory.occurredAt));
     return <div key={memory.memoryId} className="rounded-xl bg-paper p-3">
      <p className="text-sm font-medium text-ink">{text}</p>
      <p className="mt-1 text-[11px] text-muted">{t("household_memory_source").replace("{date}",date)}</p>
     </div>;
    })}
   </div>
  </section>}
