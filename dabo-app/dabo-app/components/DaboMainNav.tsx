...
export function DaboMainNav() {
  ...
  const confirmedPinnedRef = useRef<TabKey[]>(DEFAULT_TABS);
  ...
  useEffect(() => {
    ...
    supabase.from("user_navigation_preferences").select("pinned_tabs").eq("user_id", me.user_id).maybeSingle()
      .then(({ data, error }) => {
        ...
        if (tabs.length === 4 && new Set(tabs).size === 4 && !localEditRef.current) {
          confirmedPinnedRef.current = tabs;
          setPinned(tabs);
        }
      });
  ...
  function save(next: TabKey[]) {
    ...
    // L'interface reste immédiate, mais DABO ne considère plus la préférence
    // comme enregistrée tant que Supabase n'a pas confirmé l'écriture ET la
    // relecture de la ligne. Les écritures rapides restent sérialisées.
    ...
    saveQueueRef.current = saveQueueRef.current
      ...
      .then(async () => {
        ...
        const confirmed = (data?.pinned_tabs || []).filter((key: string): key is TabKey => isTabKey(key));
        if (confirmed.length !== 4 || new Set(confirmed).size !== 4 || confirmed.some((key: TabKey, index: number) => key !== next[index])) {
          throw new Error("DABO navigation preference confirmation mismatch");
        }

        confirmedPinnedRef.current = confirmed;
        lastSaveFailedRef.current = false;
        setSaveError("");
      })
      .catch((error) => {
        lastSaveFailedRef.current = true;
        setSaveError(t("settings_error_save"));
        console.error("DABO navigation preference save failed", error);
      })
      .finally(() => {
        pendingSavesRef.current = Math.max(0, pendingSavesRef.current - 1);
        if (pendingSavesRef.current === 0) setSavingPreference(false);
      });
  }
  ...
  async function finishEditing() {
    const saved = await waitForPreferenceSave();
    if (!saved) return;
    setEditing(false);
  }
  ...
  return <>
    <nav className="dabo-main-nav fixed bottom-0 left-0 right-0 z-40 bg-paper border-t border-borderLight" aria-label={t("nav_main_aria")}>
      <div className="dabo-main-nav-grid max-w-lg md:max-w-3xl mx-auto grid grid-cols-6">
        {navItems.map(item => { const Icon=item.icon; const active=isActive(item.href); return <button key={item.key} onClick={()=>go(item.href)} aria-current={active?"page":undefined} aria-label={item.label} className="dabo-main-nav-button group flex min-h-[64px] min-w-0 flex-col items-center justify-center px-0.5 py-2"><span className={`dabo-main-nav-pill ${active?"dabo-main-nav-pill-active":""} flex min-w-0 flex-col items-center justify-center gap-1 rounded-xl px-1.5 py-1.5 transition-colors ${active?"bg-ink text-paper shadow-sm":"text-muted group-hover:bg-white2 group-hover:text-ink"}`}><Icon size={21} strokeWidth={active?2.5:2}/><span className={`dabo-main-nav-label ${item.key==="today"?"dabo-main-nav-label-today":""} ${item.key==="calendar"?"dabo-main-nav-label-calendar text-[8.5px]":"text-[9.5px]"} leading-tight ${active?"font-semibold":"font-medium"}`}>{item.label}</span></span></button> })}
