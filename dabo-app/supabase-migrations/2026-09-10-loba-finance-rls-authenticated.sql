-- DABO — LOBA Finance : exécution au nom de l’utilisateur + RLS
-- Migration ciblée. Ne jamais exécuter supabase-schema.sql en production.
--
-- La lecture et les écritures Finance de la route LOBA utilisent désormais
-- le JWT de l’utilisateur authentifié. Les politiques RLS existantes sur
-- finance_transactions / finance_bills / finance_budgets restent donc actives.
-- Le paiement LOBA est exposé à authenticated, mais le RPC vérifie lui-même
-- que le JWT courant correspond bien à p_actor_user_id avant toute écriture.
...
begin
  if auth.uid() is null or auth.uid() <> p_actor_user_id then
    raise exception 'LOBA authenticated user mismatch';
  end if;
...
revoke all on function public.loba_pay_finance_bill(uuid,uuid,uuid,uuid,date,timestamptz) from service_role;
grant execute on function public.loba_pay_finance_bill(uuid,uuid,uuid,uuid,date,timestamptz) to authenticated;
