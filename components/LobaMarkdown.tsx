...
export function LobaMarkdown({text}:{text:string}){
 ...
 for(let i=0;i<lines.length;){
  ...
  const h=line.match(/^(#{1,3})\s+(.+)$/);if(h){const cls=h[1].length===1?"text-base":"text-sm";out.push(<div key={i} className={`${cls} font-semibold mt-3 mb-1`}>{inline(h[2])}</div>);i++;continue}
