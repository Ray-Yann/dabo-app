...
test("Finance V1.1 ajoute une modification discrète sur chaque dépense", () => {
  ...
  assert.match(page, /aria-label=\{`\$\{editLabel\} \$\{title\}`\}/);
  assert.match(page, /editLabel=\{t\("finance_edit"\)\}/);
...
test("Finance V1.1 préremplit les champs avec la dépense sélectionnée", () => {
  ...
  assert.match(page, /setAmount\(money\(Number\(tx\.amount\),\s*locale\)\)/);
...
test("Finance V1.1 met à jour la dépense existante sans la recréer", () => {
  ...
  assert.match(page, /editingExpense\?t\("finance_save_changes"\):t\("finance_save"\)/);
});
